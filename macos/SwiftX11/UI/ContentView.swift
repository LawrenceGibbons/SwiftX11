    ...
    VStack(alignment: .leading, spacing: 12) {
      ...
      HStack {
        ...
        Toggle("XInput2", isOn: $settings.xi2Advertised)
          .toggleStyle(.switch)
          .help("Advertise the XInputExtension (XI2). ON (default) gives clients xorg-style XI2 input; OFF keeps Electron/GTK apps on the core input path if something misbehaves. Keep XKEYBOARD on with it: GTK3 needs XKB when it receives keys over XI2. Restart X11 clients after changing.")
        Toggle("XKEYBOARD", isOn: $settings.xkbAdvertised)
          .toggleStyle(.switch)
          .help("Advertise the XKEYBOARD extension (default on). Clients then fetch the keymap with XkbGetMap instead of the core GetKeyboardMapping. Required for GTK3 dialogs to accept typed input while XInput2 is on. Restart X11 clients after changing.")
        Toggle("⌘ as Ctrl", isOn: $settings.cmdAsControl)
          .toggleStyle(.switch)
          .help("Make the Command key act as X11 Control, so ⌘C / ⌘V / ⌘A work in X11 apps like elsewhere on the Mac (default on). OFF maps ⌘ to Super/Mod4 (the X11 convention). The served keymap refreshes when X11 clients reconnect.")
