...
final class X11MetalRenderer {
  ...
  func draw(on view: MTKView) {
    ...
    // ---- HARD GATES (fixes CAMetalLayer "2x0" churn) ----
    // MTKView.drawableSize is in pixels.
    let ds = view.drawableSize
    guard ds.width >= 1, ds.height >= 1 else { return }
    
    // If we aren't attached to a window yet, don't try to render.
    // (currentDrawable tends to be nil anyway, but be explicit.)
    ...
    guard let render = cmdBuf.makeRenderCommandEncoder(descriptor: rpd) else {
      inFlight = false
      return
    }
    render.endEncoding()
    ...
    cmdBuf.addCompletedHandler { [weak self, weak view] _ in
      ...
      DispatchQueue.main.async {
        ...
        // Re-check readiness BEFORE re-triggering a draw.
