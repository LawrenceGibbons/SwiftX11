...
3. **Flip XKEYBOARD and XInput2 to default ON** — Phase F verified 2026-09-07 (Mac: libX11, xkbcomp round-trip; container: xterm, Vivado, Vitis, portal-GTK dialog over XI2); both toggles still default OFF in `SettingsStore.swift`
4. **XI2 Stage 2** — `docs/XI2_XORG_COMPARISON.md` §6 Phases C–E: per-window per-client masks + fan-out (M4 full, M5), focus/crossing detail (M9, M15), protocol hygiene (M12–M14); XKB StateNotify on Caps Lock
