...
- **v1.20.0.22 — XInput2 and XKEYBOARD default ON**: both `SettingsStore` defaults and the C++ atomics flipped to 1 (persisted user values still win). Phase B is complete with this: its grab tests passed on 2026-09-04 and its last failure was M23. Toggling either OFF remains the escape hatch; keep XKEYBOARD on whenever XI2 is on (GTK3 needs XKB to receive keys over XI2).
...
1. **XI2 Stage 2** — `docs/XI2_XORG_COMPARISON.md` §6 Phases C–E: per-window per-client masks + fan-out (M4 full, M5), focus/crossing detail (M9, M15, M18, M19), protocol hygiene (M11–M14, M20, M21, L items). Phases A, B and F are done and default ON (v1.20.0.22); the Ctrl+click regression (v1.19.36.16) and grab `owner_fd` (v1.19.36.17, B2) items are closed.
