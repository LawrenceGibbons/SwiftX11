...
1. **XI2 Stage 2** — `docs/XI2_XORG_COMPARISON.md` §6: Phase C (M4 full, M5) landed in v1.20.0.23, verification pending; then Phase D focus/crossing detail (M9, M15, M18, M19) and Phase E protocol hygiene (M11–M14, M20, M21, L items). Phases A, B and F are done and default ON (v1.20.0.22); the Ctrl+click regression (v1.19.36.16) and grab `owner_fd` (v1.19.36.17, B2) items are closed.
