...
**CHECKPOINT (2026-09-09):** Phases 1–3 — the entire "real root window core" (§6) — are implemented, gate-passed on v2.0.0.5-dbg, and marked by branch `r1-phase3-verified` (develop @ 1b4ac2b). Per the user, R1 is **paused here**: Phase 4 (SendEvent-to-root + EWMH ClientMessage) is deliberately deferred as its own later increment (§6 always allowed this: "Phase 4 is the EWMH payoff and can be its own release increment"). It is **not started, not abandoned** — resume from this plan. develop stays at 2.0.0.5-dbg (DEBUG_BUILD unflipped; the v2.0.0 release is the user's call when they choose to ship R1).
