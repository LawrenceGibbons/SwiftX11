...
| xeyes stops tracking — **predicted** from D-3/D-4/W-1, **not yet observed** (xeyes tracked normally alongside Vitis on 2026-09-04) | M4, M5. The sequence the analysis predicts to fail: start xeyes *then* open a portal-GTK dialog (GDK's root `XISelectEvents` overwrites the global mask — the last selector wins, so order matters); or move the pointer across a Vitis window and back onto the desktop (`activeXid` stays on Vitis and RawMotion goes to its fd) | `xinput test-xi2 --root` in a second client while xeyes runs; `[WIRE]` RawMotion GenericEvents (type 35, evtype 17) going to a fd that never selected them |
...
| **xeyes** | `XI_RawMotion` on root via libXi | Predicted (not yet observed) to freeze when a later client's root selection clobbers the global mask, or while `activeXid` is another client's window; RawMotion never cleared after exit | M4, M5 |
