...
BdfParseResult parsePcf(const uint8_t* data, size_t len, BdfFont& out) {
  ...
  if (const TocEntry* t = findTable(PCF_BITMAPS)) {
    if (t->offset + 4 <= len) {
      ...
      bool bitmapMsbFirst = (fmt & PCF_BIT_MASK) != 0;
      base += 4;
      ...
      if (base + 4 <= data + len) {
        ...
        // Our text rendering uses MSB-first bit order (bit 7 = leftmost pixel).
        // If the PCF font uses LSB-first, we need to reverse bits in each byte
        // when extracting glyph bitmaps below.
        if (!bitmapMsbFirst && bitmapData && bitmapDataSize > 0) {
          // Build a reversed copy of the bitmap data
          ...
          // Copy and reverse in-place in our local buffer
          // (we'll reference bitmapData from this static buffer)
          static thread_local std::vector<uint8_t> reversedBitmaps;
          reversedBitmaps.resize(bitmapDataSize);
          for (size_t i = 0; i < bitmapDataSize; i++) {
            reversedBitmaps[i] = bitrev[bitmapData[i]];
          }
          bitmapData = reversedBitmaps.data();
        }
      }
    }
  }
