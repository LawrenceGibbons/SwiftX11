...
void XProtoDaemon::sendXkbStateNotify(uint16_t changed, const uint8_t evTemplate[32]) {
  if (!server_ || changed == 0 || !evTemplate) return;
  for (auto& [fd, cs] : clients_) {
    (void)fd;
    if (!cs.client || !cs.client->xkb().initialised) continue;
    if ((cs.client->xkb().stateNotifyMask & changed) == 0) continue;
    uint8_t fixed[32];
...
void XProtoDaemon::removeClient(int fd) {
  ...
  // (R1 Phase 3: this covers the root window's XI2 selections too — the
  // RawMotion gate no longer keeps a dead client's bits alive.)
  server_->ctx().windows().removeClientMasks(fd);
